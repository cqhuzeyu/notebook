...

{
    char *s = this.string;
    if(!strcmp(s, "const")) {
        __builtin___strcpy_chk (s, "read-only", __builtin_object_size (s, 2 > 1 ? 1 : 0));
        return QUALIFIER;
    }
...
void gettoken(void)
{
    ...
    if (*p == '*') {
        __builtin___strcpy_chk (this.string, "pointer to", __builtin_object_size (this.string, 2 > 1 ? 1 : 0));
        this.type = '*';
        return;
    }
    this.string[1] = '\0';
